...
class YoloDet:
    ...
    def infer(self, img, **kwargs):
        ...
        pre = self.model.forward(self.outlayer_names)
        result = self.img_postprocess(pre, ori_w / new_w, ori_h / new_h, left, top, score)
        return result
...
class YoloSeg:
    ...
    def infer(self, img, **kwargs):
        ...
        self.model.setInput(img)
        predict_maps = self.model.forward(self.outlayer_names)
    ...
    def img_postprocess(self, predict_maps):
        ...
        # 将区域内的值复制到过滤后的掩码中
        filtered_mask[small_y_min:small_y_max, small_x_min:small_x_max] = mask[small_y_min:small_y_max, small_x_min:small_x_max]

        # 使用 OpenCV 进行放大，保持边缘清晰
        resized_mask = cv2.resize(filtered_mask, (800, 800), interpolation=cv2.INTER_CUBIC)
        return resized_mask
...
class PPLCNet:
    ...
    def infer(self, img, **kwargs):
        ...
        self.model.setInput(img)
        label= self.model.forward(self.outlayer_names)[0]
        label = label[None, :]
